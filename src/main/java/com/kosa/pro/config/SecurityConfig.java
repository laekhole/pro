...
import com.kosa.pro.config.auth.AuthFailureHandler;
import com.kosa.pro.config.auth.AuthSucessHandler;
import com.kosa.pro.config.auth.PrincipalDetailsService;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Resource
	private GlobalProperty _global;
...
//			.antMatchers("/", "/auth/**", "/api/auth/**", "/js/**","/css/**","/images/**").permitAll() // 해당 경로들은 접근을 허용
...
//			.antMatchers("/admin/**").hasAnyAuthority("ROLE_ADMIN") //ADMIN 권한만 허가  
		...
		.and()
			.formLogin() // 로그인 폼은
			.loginPage("/auth/loginForm") // 해당 주소로 로그인 페이지를 호출한다.
			.loginProcessingUrl("/auth/login") // 해당 URL로 요청이 오면 스프링 시큐리티가 가로채서 로그인처리를 한다. -> loadUserByName
			.successHandler(authSucessHandler) // 성공시 요청을 처리할 핸들러
			.failureHandler(authFailureHandler) // 실패시 요청을 처리할 핸들러
		.and()
			.logout()
			.logoutRequestMatcher(new AntPathRequestMatcher("/logout")) // 로그아웃 URL
		    .logoutSuccessUrl("/auth/loginForm") // 성공시 리턴 URL
		...
		.and()
            ...
            .expiredUrl("/auth/loginForm?error=true&exception=Have been attempted to login from a new place. or session expired")  // 세션이 만료된 경우 이동 할 페이지를 지정
        .and()
...
//		System.out.println("user1@aaa.com->[" + encoder.encode("123"));
//	}
}
