  ...
  <body>
    ...
    <script type="text/javascript">
    	...
    	const roomListOutput = () => {
        	$.ajax({ 
        		type: "GET",
        		url: '<c:url value="/admin/FindAllRoom"/>',
        ...
        const param = {
       	...
       	$.ajax({ 
       		type: "POST",
       		url: '<c:url value="/admin/CreateRoom"/>',
       		data: param, 
       		dataType: "json" // 요청을 서버로 해서 응답이 왔을 때 기본적으로 모든 것이 문자열 (생긴게 json이라면) => javascript오브젝트로 변경
       	}).done(resp => {
        ...
        localStorage.setItem('adminchat.roomId', roomId);
        location.href = '<c:url value="/admin/CreateRoom/enter/"/>' + roomId;
    	}	    
    </script>
	</body>
</html>